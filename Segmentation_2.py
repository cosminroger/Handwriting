...
file = sys.argv[1]
img = rgb2gray(imread(file))    #Read image
...
for x in range(len(peaks)):     #Crop each line and save as different image
	...
	plt.axis('off')
	plt.savefig('Segmented_Lines\\Line_'+str(x+1)+'_image.png', bbox_inches='tight',pad_inches = 0,dpi=300)
...
image_array = sorted(glob.glob('Segmented_Lines/*.png'),key=os.path.getmtime)
#Get address of all lines present in image

no_of_lines = len(image_array)

for x in range(no_of_lines):
	os.mkdir(curr_dir+"/Segmented_Characters_Per_Lines\\"+str(x+1))
...
for x in image_array:
	...
	for y in range(len(peaks)-1):
		...
		if square:
			name = 'Segmented_Characters_Per_Lines\\'+str(counter)+'\\char_'+str(y+1)+'.png'
			...
			plt.savefig('Segmented_Characters_Per_Lines\\'+str(counter)+'\\char_'+str(y+1)+'.png', bbox_inches='tight',pad_inches = 0,dpi=100)
		
		print("Cropped line: "+str(counter)+" character: "+str(y+1)+"/"+str(len(peaks)-1))
...
alpha_direct = glob.glob('Segmented_Characters_Per_Lines/*')
