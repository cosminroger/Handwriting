...
sorted_img_list = [[int(n) for n in re.findall(r"\d+", f)] for f in img_list]
sorted_img_list = [x for _,x in sorted(zip(sorted_img_list,img_list))]
img_list_len = len(sorted_img_list)


for img_number in range(img_list_len):

	img = rgb2gray(imread(sorted_img_list[img_number]))    #Read image
	...
	peaks, _ = find_peaks(hpp, distance=80, height = 40)    #Find local maxima peaks in plot
	#Block only for debugging
	if debug == True:
		plt.plot(hpp)
	...
	sorted_image_array = [[int(n) for n in re.findall(r"\d+", f)] for f in image_array]
	sorted_image_array = [x for _,x in sorted(zip(sorted_image_array,image_array))]

	no_of_lines = len(sorted_image_array)
	#print(no_of_lines)
